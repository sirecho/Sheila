...
std::vector<Position> Pawn::getLegalMoves(Board *board) {
    ...
    if (this->side_ == WHITE && position_.number() == WHITE_PAWN_ROW && board->pieceAt(position_.stepNorth(2)) == NULL && squareBPiece == NULL) {
        moves.push_back(position_.stepNorth().stepNorth());
    } else if (this->side_ == BLACK && position_.number() == BLACK_PAWN_ROW && board->pieceAt(position_.stepSouth(2)) == NULL && squareBPiece == NULL) {
